...
def build_merged_streaming() -> str:
    ...
    try:
        for store in stores_all:
            ...
            # Każdy rekord modelowy ma reprezentować jeden sklep, produkt i dzień.
            sales_key_cols = ["StoreNo", "IDX", "DateNo"]
            sales_dupes = sales.duplicated(sales_key_cols, keep=False)
            if sales_dupes.any():
                sample = sales.loc[sales_dupes, sales_key_cols].head(10)
                raise ValueError(
                    f"[{store}] Sales zawiera {int(sales_dupes.sum())} wierszy "
                    f"w zduplikowanych StoreNo+IDX+DateNo. Przykład:\n{sample}"
                )
            ...
            # 4.6) BUDOWA PEŁNEGO PANELU DZIENNEGO DLA AKTYWNYCH PRODUKTÓW
            #
            # W poprzedniej wersji bazą był `sales`, więc dzień bez sprzedaży
            # w ogóle nie trafiał do zbioru. To psuło szeregi czasowe i lagi.
            #
            # Teraz bazą są dni aktywności produktu z planogramu.
            # Dodatkowo dokładamy klucze istniejące w sprzedaży, aby nie zgubić
            # rzeczywistych obserwacji, których ewentualnie nie ma w planogramie.

            key_cols = ["StoreNo", "IDX", "DateNo"]
            selected_idxs = set(sales["IDX"].astype(str).unique())

            panel_parts = []

            if not planogram.empty and all(k in planogram.columns for k in key_cols):
                planogram = planogram[planogram["IDX"].isin(selected_idxs)].copy()
                panel_parts.append(planogram[key_cols].drop_duplicates())

            # Zachowujemy również każdy dzień, w którym faktycznie wystąpiła sprzedaż.
            panel_parts.append(sales[key_cols].drop_duplicates())
            ...
            # Kontrola: jedna obserwacja = jeden sklep, produkt i dzień.
            if panel.duplicated(key_cols).any():
                raise ValueError(f"[{store}] Duplikaty w panelu StoreNo+IDX+DateNo")

            # Do aktywnego panelu dokładamy dane sprzedażowe.
            chunk = panel.merge(sales, on=key_cols, how="left", validate="one_to_one")

            # Jeśli produkt był aktywny w planogramie, ale nie ma rekordu sprzedaży,
            # interpretujemy to jako sprzedaż równą zero.
            chunk["Sales"] = pd.to_numeric(chunk["Sales"], errors="coerce").fillna(0)

            if "SalesValue" in chunk.columns:
                chunk["SalesValue"] = pd.to_numeric(
                    chunk["SalesValue"], errors="coerce"
                ).fillna(0)

            # + idxs (małe)
            if not idxs.empty:
                chunk = chunk.merge(idxs, on="IDX", how="left", validate="many_to_one")

            # + planogram (np. stock_price_net)
            if not planogram.empty and all(k in planogram.columns for k in key_cols):
                planogram_for_merge = planogram.drop_duplicates(subset=key_cols)
                chunk = chunk.merge(
                    planogram_for_merge,
                    on=key_cols,
                    how="left",
